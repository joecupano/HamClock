...
# ask web-only
if ask "build for web access only (no hardware display)" ; then
...
WC0=$(wc -l < $LOGFN)
echo running make $HC_BUILD >> $LOGFN
make $HC_BUILD >> $LOGFN 2>&1 &
job=$!
...
# icon?
if [ -d $HOME/Desktop ] && [ $HC_WEBONLY -eq 0 ] ; then
...
# User Guide?
if [ -d ~/Desktop ] && [ $HC_WEBONLY -eq 0 ] ; then
    UGFN=HamClockKey.pdf
    DTFN=$HOME/Desktop/$UGFN
    if ask "install User Guide on desktop" ; then
        UGURL=https://clearskyinstitute.com/ham/HamClock/$UGFN
        if ! curl --silent --show-error --output $DTFN $UGURL >> $LOGFN 2>&1 ; then
            inform Error downloading $UGURL -- see $LOGFN
        fi
    else
        rm -f $DTFN
    fi
...
if [ -d $MPATH ] && [ $HC_WEBONLY -eq 0 ] && ask "install HamClock man page" ; then
...
if ask "start HamClock automatically each time Pi is booted" ; then
    if [ $HC_WEBONLY -eq 0 ] ; then
        ...
        CTBTMP=.x.crontab
        crontab -l > $CTBTMP
        if grep -q hamclock $CTBTMP ; then
            inform hamclock is already in crontab
        else
            inform adding hamclock to crontab
            echo "@reboot /usr/local/bin/hamclock" >> $CTBTMP
            crontab $CTBTMP
        fi
        rm $CTBTMP
    fi
    ...
    if [ $HC_WEBONLY -eq 0 ] ; then
        ...
        CTBTMP=.x.crontab
        crontab -l | sed -e /hamclock/d > $CTBTMP
        crontab $CTBTMP
    fi
fi
